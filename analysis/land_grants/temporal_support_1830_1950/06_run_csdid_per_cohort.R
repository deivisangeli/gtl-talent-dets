###############################################################################
# CSDID event studies with per-cohort estimation.
# Each cohort (g) is estimated on its own subpanel (treated of that cohort +
# all runner_ups pooled). Reuses same balance filter and covariate as script 03.
# Produces one figure per (outcome, timing, mode) with facet_wrap by cohort.
###############################################################################
...
results_subdir <- "amws_county_pairs_temporal_support_1830_1950_baseline_pop_per_cohort"
...
for (timing_name in names(panel_by_timing)) {
  ...
  cohorts <- sort(unique(panel_t$g[panel_t$g > 0]))
  for (outcome_col in outcomes) {
    for (cohort in cohorts) {
      message("Running per-cohort CSDID: ", timing_name, " | ", outcome_col,
              " | g = ", cohort)
...
status <- dynamic_att %>%
  distinct(outcome, timing, cohort, n_treated, n_control) %>%
  arrange(outcome, timing, cohort)
write_csv(status, results_subdir_path("status.csv"), na = "")

# Plot: one figure per (outcome, timing) with facet_wrap(~cohort)
for (o in outcomes) {
  for (t in names(panel_by_timing)) {
    ...
    ncol_facet <- if (n_cohorts <= 3) n_cohorts else 3L
    p <- ggplot(df_plot, aes(x = event_time, y = estimate,
      ...
      facet_wrap(~ cohort_label, ncol = ncol_facet, scales = "free_y") +
      ...
      labs(
        ...
        title = str_wrap(
          paste0(outcome_labels[[o]],
                 " - Controlled by baseline births (per-cohort CSDID, ref e=0",
    ...
    n_rows_facet <- ceiling(n_cohorts / ncol_facet)
    ggsave(
      results_subdir_path(paste0("ES_", o, "_", t, ".png")),
      p,
      width = 3 * ncol_facet + 2,
      height = 2.5 * n_rows_facet + 2,
      dpi = 300
    )
  }
}
...
notes <- c(
  "Per-cohort CSDID: each treatment cohort g estimated separately on its own",
  "subpanel (treated of cohort g + all never-treated runner_ups pooled).",
  paste0("Balance mode: ", balance_mode, ". Event-time window: [",
