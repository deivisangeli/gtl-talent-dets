...
# Goal: Controlled AMWS event studies using Andrews college site-selection pairs
###############################################################################
...
results_subdir <- "amws_county_pairs_all_colleges_controls"
...
build_stacked_panel <- function(timing_var, timing_name) {
 event_sample <- event_lookup %>%
  ...
  filter(
   event_treatment_decade > first_panel_decade,
...
sample_summary <- model_status %>%
 filter(ok) %>%
 select(outcome, spec, timing, n_events, n_units, n_rows, n_treated_units,
...
notes_lines <- c(
 "Controlled AMWS event study using Andrews high-quality college site-selection experiments",
