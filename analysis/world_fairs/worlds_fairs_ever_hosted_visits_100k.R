...
#       world's fair hosting as treatment, restricted to fairs with at least
#       100,000 visits.
#
# Treatment:
#   - First realized world's fair venue with >=100,000 visits located inside
#     the unit polygon.
#   - Controls are units that never host a qualifying fair over 1790-1961.
...
#   Rscript analysis/world_fairs/worlds_fairs_ever_hosted_visits_100k.R
###############################################################################
...
results_dir <- file.path(
  ...
  "results",
  "worlds_fair",
  "worlds_fairs_uk_us_ever_hosted_visits_100k_event_studies_with_london_events_1840_1910"
)
...
extract_dynamic_att <- function(es, outcome, spec_label) {
  tibble(
    outcome = outcome,
...
extract_simple_att <- function(ag, outcome, spec_label) {
  tibble(
    outcome = outcome,
    treatment_spec = spec_label,
    control_group = control_group_name,
...
plot_dynamic_event_study <- function(es, outcome, spec_label, y_limits, sample_annotation) {
  did::ggdid(es) +
    annotate(
      "label",
      x = Inf,
    ...
    labs(
      ...
      title = str_wrap(
        paste(
          "World's fairs pooled UK+US hosted-only event study, visits >= 100,000",
          spec_label,
          outcome
        ),
        width = 72
...
build_host_exposure_one_country <- function(targets_sf, venues_country) {
  ...
  hosted_audit <- tibble(
    ...
    mutate(
      treatment_spec = .env$treatment_spec,
      hosted_unit = TRUE
    ) %>%
...
build_host_exposure <- function(uk_targets, us_targets, venues) {
  uk_exposure <- build_host_exposure_one_country(
    uk_targets,
    venues %>% filter(host_matched_country_iso3 == "GBR")
  )
  us_exposure <- build_host_exposure_one_country(
    us_targets,
    venues %>% filter(host_matched_country_iso3 == "USA")
  )

  list(
    hosted_audit = bind_rows(uk_exposure$hosted_audit, us_exposure$hosted_audit),
...
run_event_study <- function(data, outcome, spec_label, window = event_window, cores = 4) {
  data_es <- data %>%
    select(
      unit_num,
      unit_id,
...
###############################################################################
# Build hosted-only treatment assignment
###############################################################################

message("Building conservative hosted-only treatment assignment for fairs with >= 100,000 visits...")
venue_data <- load_conservative_venues()
venues <- venue_data$venues
venue_quality_audit <- venue_data$audit
exposure <- build_host_exposure(uk_targets, us_targets, venues)
...
write_csv(hosted_audit, file.path(results_dir, "hosted_venue_match_audit.csv"))
...
###############################################################################
# Run hosted-only event studies
###############################################################################

message("Running hosted-only event study...")
spec_label <- treatment_spec
spec_dir <- file.path(results_dir, treatment_dir)
...
if (length(successful_models) > 0L) {
  dynamic_att <- imap_dfr(
    successful_models,
    ~ extract_dynamic_att(.x$es, .x$outcome, .x$treatment_spec)
  )
  simple_att <- imap_dfr(
    successful_models,
    ~ extract_simple_att(.x$simple, .x$outcome, .x$treatment_spec)
  )
} else {
  dynamic_att <- tibble()
  simple_att <- tibble()
...
write_csv(hosted_audit, file.path(spec_dir, "hosted_venue_match_audit.csv"))
...
write_csv(dynamic_att, file.path(spec_dir, "dynamic_att.csv"))
write_csv(simple_att, file.path(spec_dir, "simple_att.csv"))

write_csv(treated_units, file.path(results_dir, "treatment_assignment.csv"))
...
write_csv(country_summary, file.path(results_dir, "sample_summary_by_country.csv"))
write_csv(effective_sample_summary, file.path(results_dir, "effective_sample_summary_by_outcome.csv"))
write_csv(model_status, file.path(results_dir, "model_status.csv"))
write_csv(dynamic_att, file.path(results_dir, "dynamic_att.csv"))
write_csv(simple_att, file.path(results_dir, "simple_att.csv"))
...
if (nrow(dynamic_att) > 0L) {
  ...
  for (model in successful_models) {
    ...
    ggsave(
      file.path(spec_dir, paste0("ES_", sanitize_filename(model$outcome), "_hosted.png")),
      plot_es,
      width = 8,
      height = 6,
      dpi = 300
...
notes <- c(
  ...
  "A unit is treated only when a realized fair venue point with visits >= 100,000 intersects its polygon.",
  "Nearby units are not treated unless they also contain a qualifying venue point.",
  ...
  "Always-treated units are units first hosting a qualifying fair before 1840.",
  "Future-treated units are units first hosting a qualifying fair after 1910 and before or during 1961.",
...
root_notes <- c(
  "World's fairs pooled UK historical urban-unit + US county hosted-only event studies, visits >= 100,000",
  "",
  paste0("Run timestamp: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
  ...
  paste0("Visits threshold: >= ", visits_threshold_label),
  "Treatment specification: hosted only, based on qualifying venue point intersecting the unit polygon.",
  ...
  "Control group is strictly never hosted by fairs with visits >= 100,000 over 1790-1961.",
