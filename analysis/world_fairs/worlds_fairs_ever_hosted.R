...
#       world's fair hosting as treatment.
#
# Treatment:
#   - First realized world's fair venue located inside the unit polygon.
#   - Controls are units that never host a fair over 1790-1961.
...
#   Rscript analysis/world_fairs/worlds_fairs_ever_hosted.R
###############################################################################
...
results_dir <- file.path(
  ...
  "results",
  "worlds_fair",
  "worlds_fairs_uk_us_ever_hosted_event_studies_with_london_events_1840_1910"
)
...
plot_dynamic_event_study <- function(es, outcome, spec_label, y_limits, sample_annotation) {
  did::ggdid(es) +
    ...
    labs(
      ...
      title = str_wrap(
        paste(
          "World's fairs pooled UK+US hosted-only event study",
          spec_label,
          outcome
        ),
        width = 72
...
message("Building conservative hosted-only treatment assignment...")
...
notes <- c(
  ...
  "A unit is treated only when a realized fair venue point intersects its polygon.",
  "Nearby units are not treated unless they also contain a venue point.",
  ...
  "Always-treated units are units first hosting before 1840.",
  "Future-treated units are units first hosting after 1910 and before or during 1961.",
...
root_notes <- c(
  "World's fairs pooled UK historical urban-unit + US county hosted-only event studies",
  "",
  paste0("Run timestamp: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
  ...
  "Treatment specification: hosted only, based on venue point intersecting the unit polygon.",
  ...
  "Control group is strictly never hosted over 1790-1961.",
