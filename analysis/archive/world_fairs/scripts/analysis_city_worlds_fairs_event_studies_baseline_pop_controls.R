...
# Goal: City-level event studies using world's fairs as treatment, controlling
...
#
# Run from analysis/:
#   Rscript analysis_city_worlds_fairs_event_studies_baseline_pop_controls.R
###############################################################################
...
results_dir <- file.path("results", "worlds_fairs_city_event_studies_baseline_pop_controls")
...
write_csv(
  fair_match_audit,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_match_audit.csv")
)
write_csv(
  treatment_assignment,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_treatment_assignment.csv")
)
...
missing_controls <- imap_dfr(control_sets, function(controls, control_spec) {
  bind_rows(
    panel_decade %>%
      ...
      mutate(sample = "combined_us_europe", control_spec = control_spec),
    panel_decade %>%
      filter(unit_type == "europe_city") %>%
      distinct(unit_id, across(all_of(controls))) %>%
      summarise(missing_units = sum(if_any(all_of(controls), is.na)), .groups = "drop") %>%
      mutate(sample = "europe_city_only", control_spec = control_spec)
  ) %>%
...
write_csv(sample_summary, file.path(results_dir, "worlds_fairs_baseline_pop_controls_sample_summary.csv"))
write_csv(missing_controls, file.path(results_dir, "worlds_fairs_baseline_pop_controls_missing_controls.csv"))
write_csv(
  event_distribution,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_event_distribution.csv")
)
...
if (length(successful_models) == 0) {
  ...
  write_csv(
    model_status_failed,
    file.path(results_dir, "worlds_fairs_baseline_pop_controls_model_status.csv")
  )
...
write_csv(
  dynamic_att,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_dynamic_att.csv")
)
write_csv(
  simple_att,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_simple_att.csv")
)
write_csv(
  model_status,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_model_status.csv")
...
write_csv(
  hyphen_year_check,
  file.path(results_dir, "worlds_fairs_baseline_pop_controls_multi_year_fair_year_check.csv")
)

notes <- c(
  "World's fairs city-level event studies with baseline population controls",
  "",
