...
# Goal: AMWS event studies from 1900 using Andrews college site-selection experiments
###############################################################################
...
results_subdir <- "amws_county_pairs_all_colleges_from_1900"
...
sample_summary <- bind_rows(
 tibble(
  ...
  retained_events = n_distinct(panel_es_std$event_id),
  already_treated_events = n_distinct(
   panel_es_std$event_id[
    panel_es_std$g > 0 & panel_es_std$g <= first_panel_decade
   ]
  ),
  estimable_events = n_distinct(
   panel_es_std$event_id[panel_es_std$g > first_panel_decade]
  ),
 ...
 tibble(
  ...
  original_events = nrow(event_lookup),
  retained_events = n_distinct(panel_es_shift$event_id),
  already_treated_events = n_distinct(
   panel_es_shift$event_id[
    panel_es_shift$g > 0 & panel_es_shift$g <= first_panel_decade
   ]
  ),
  estimable_events = n_distinct(
   panel_es_shift$event_id[panel_es_shift$g > first_panel_decade]
  ),
...
notes_lines <- c(
 "AMWS event study from 1900 using Andrews high-quality college site-selection experiments",
