...
# Pivot to long: one row per county × decade
pop_long <- pop_raw %>%
...
# Aggregate: n_inventors by county × decade
inventors_agg <- inventors_county %>%
 count(GEOID, decade, name = "n_inventors")

###############################################################################
# 5. Build balanced panel: county × decade
###############################################################################

# All county × decade combinations
...
ggsave("Results County/county_density_aggregate.png", plot = plot_agg,
