...
# Uploading databases
###############################################################################
#data <- read_csv("Output/us_panel.csv", show_col_types = FALSE)
data_full <- read_csv("Output/us_panel_fixed.csv", show_col_types = FALSE)
facilities <- read_delim("Output/facilities_us.csv",delim = ";",
                         locale = locale( decimal_mark = ".", grouping_mark = ""),
                         show_col_types = FALSE)

facilities_alt <- read_delim("Output/facilities_us_alt.csv",delim = ";",
                         locale = locale( decimal_mark = ".", grouping_mark = ""),
                         show_col_types = FALSE)
###############################################################################
...
# Creating a dataframe for the treated cells
#hyde_cells <- hyde_cells %>%
# left_join(data_full_es_decade %>% select(cell_id, decade, inv_per_100k, g_std),
#  by = "cell_id") %>%
# mutate(treated = g_std > 0)

#treated_cells <- data_full_es_decade %>%
# select(cell_id, g_std) %>%
# distinct() %>%
# mutate(treated = g_std > 0)

#hyde_cells <- hyde_cells %>%
# left_join(treated_cells, by = "cell_id")
...
#plot_treatment_2 <- ggplot() +
# geom_sf(data = us_ll, fill = "gray95", color = "gray70") +
# geom_sf(data = hyde_ll %>% filter(!treated), fill = NA, linewidth = 0.15,
#         alpha = 0.3, color = "red") +
# geom_sf( data = hyde_ll %>% filter(treated), fill = "blue", color = NA,
 #         alpha = 0.5) +
# geom_sf(data = buffer_ll, fill = NA, linewidth = 0.6) +
# geom_sf(data = fac_ll, size = 1) +
# coord_sf(xlim = c(-125, -65), ylim = c(25, 50)) +
# theme_minimal() +
 #labs(title = "HYDE Cells and Facilities")
...
# coord_sf(xlim = c(-125,-65), ylim = c(25,50)) +
# theme_minimal() +
# labs(title = "Inventors per 100k — All Decades (Mean) - Only places with positive values",
#  fill = "Inv / 100k")

###############################################################################
# Saving Results
###############################################################################
ggsave(filename = "Results US/treatment_map.png",plot=plot_treatment, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/density.png",plot= plot_decade_density_all, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/ES_US_full_std.png",plot= decade_50_std, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/ES_US_full_alt.png",plot= decade_50_alt, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/ES_US_selected_std.png",plot= es_alt_facilities_std, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/ES_US_selected_alt.png",plot= es_alt_facilities_alt, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/plot_means.png",plot= plot_means_full, width = 8,
       height = 6, dpi = 300)

ggsave(filename = "Results US/plot_means_focus.png",plot= plot_means_focus, width = 8,
       height = 6, dpi = 300)
