...
$TalentDetsDataDir = if ($env:TALENT_DETS_DATA_DIR) {
    $env:TALENT_DETS_DATA_DIR
} else {
    'C:/Users/deivi/Globtalent Dropbox/gtl_talent_dets'
}

if (-not $InputFile) {
    $InputFile = Join-Path $TalentDetsDataDir 'output/amws/amws_1955_batches/in/00136.tsv'
}
if (-not $OutputFile) {
    $OutputFile = Join-Path $TalentDetsDataDir 'output/amws/amws_1955_batches/out/00136.jsonl'
}
