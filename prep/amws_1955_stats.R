...
out_dir <- "output/amws_1955_stats"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# ---- Load -------------------------------------------------------------------
amws_clean <- fread("output/amws_1955_cleaned.csv")
amws_geo   <- fread("output/amws_1955_us_geocoded_final.csv")
wiki       <- fread("output/crossverified_with_stem.csv")
...
wiki_panel <- fread("output/us_panel_county_stem_1790.csv")
dec_lo <- (yr_lo %/% 10) * 10
dec_hi <- (yr_hi %/% 10) * 10
