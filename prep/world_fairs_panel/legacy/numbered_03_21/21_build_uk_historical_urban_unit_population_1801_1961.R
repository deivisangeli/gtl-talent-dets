###############################################################################
# Build a harmonized UK historical urban-unit population panel, 1801-1961.
#
# Target geography:
...
#   London fragments. 1921 urban units overlapping modern Greater London are
#   removed from the target set to avoid overlapping target geometries.
#
# Sources:
...
#   intersection area weighted by the target unit's 1911 population density.
#
# Run from the repository root:
#   Rscript prep/world_fairs_panel/21_build_uk_historical_urban_unit_population_1801_1961.R
###############################################################################
...
required_files <- c(
  law_panel_file, benchmark_1911_file, boundary_gpkg, lau_gpkg
)
...
greater_london_geom <- st_union(greater_london_lau)
greater_london_sf <- st_sf(
  ...
  target_boundary_source = "GISCO 2019 LAU E090000* union",
...
base_target_intersections <- suppressWarnings(st_intersection(
  base_targets[, c("target_row", "boundary_id", "boundary_name", "boundary_type")],
  greater_london_sf[, c("target_unit_id")]
))
if (nrow(base_target_intersections) > 0L) {
  base_target_intersections$overlap_area_m2 <- as.numeric(st_area(base_target_intersections))
  london_overlap_rows <- unique(
    base_target_intersections$target_row[
      base_target_intersections$overlap_area_m2 > 1000
    ]
  )
} else {
...
target_dt[
  ...
  `:=`(
    ...
    benchmark_area_type = "Greater London",
    target_population_1911 = read_greater_london_1911()
  )
]
if (target_dt[is.na(target_population_1911), .N] > 0L) {
  missing_targets <- target_dt[is.na(target_population_1911), target_unit_name]
  stop(
    "Target units missing 1911 density population:\n",
...
cat("1921 target units removed because of Greater London overlap: ",
...
for (year in census_years_nomis) {
  ...
  allocation[, density_weight :=
    intersection_area_m2 * target_density_1911_per_m2]
  allocation[
  ...
  allocation[, `:=`(
    ...
    allocation_method = fifelse(
      total_density_weight > 0,
      "nomis_polygon_intersection_1911_density_weighted",
...
observed_agg <- observed_alloc[, .(
  ...
  share_population_density_weighted = sum(
    allocated_population[
      allocation_method == "nomis_polygon_intersection_1911_density_weighted"
    ],
    na.rm = TRUE
...
observed[, `:=`(
  ...
  population_quality = fcase(
    ...
    grepl("density_weighted", allocation_method, fixed = TRUE), "observed_nomis_1911_density_weighted",
